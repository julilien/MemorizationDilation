...
from utils import CROSS_ENTROPY_TAG
...
def visualize_class_cluster_dispersion_2d(classes, std_devs, class_means):
    for class_idx in classes:
        loc_circle = plt.Circle((class_means[class_idx, 0], class_means[class_idx, 1]), std_devs[class_idx],
                                color=CLASS_COLORS[class_idx], alpha=0.1)
...
def visualize_clf_2d(model, instances, is_binary, num_classes, label="Classifier"):
    ...
    if is_binary:
        ...
        logging.warning("Currently, decision boundary plotting is not implemented for softmax with more than two "
                        "classes.")
